...
class App extends Component {
  state = {
    ...
    seconds: 60,
    score: 0,
    imageToDisplayID: imagesList[0].id,
  }
  ...
  checkForThumbnailClick = id => {
    const {imageToDisplayID} = this.state
    if (imageToDisplayID === id) {
      const randomInt = Math.floor(Math.random() * 100) % 30
      this.setState(prevState => ({
        score: prevState.score + 1,
        matched: true,
        imageToDisplayID: imagesList[randomInt].id,
      }))
    } else {
      clearInterval(this.timerId)
      this.setState({
  ...
  render() {
    ...
    const thumbnailDisplayList = imagesList.filter(
      ...
      <div>
        ...
        {!isGameOver && (
          <>
            {matched && (
              <img
                src={
                  imagesList.filter(each => each.id === imageToDisplayID)[0]
                    .imageUrl
                }
                alt="match"
              />
            ...
            <ul>
              {thumbnailDisplayList.map(each => (
                <ThumbnailItem
                  checkForThumbnailClick={this.checkForThumbnailClick}
                  each={each}
                  key={each.id}
                />
              ))}
        ...
        {isGameOver && (
          <>
            ...
            <p>YOUR SCORE</p>
            <p>{score}</p>
            <button onClick={this.playAgainClicked}>
              <img
